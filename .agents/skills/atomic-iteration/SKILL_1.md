...
- Rules: Present only the modified lines of README using standard git diff format (max 15 lines snippet). Wait for user approval before versioning the plan through `semantic-git`. DO NOT START CODING UNTIL THE PLAN IS COMMITTED.

5. Execution
- Trigger: `reference/ifp/implementation_plan_[N].md` and `README.md` are approved and committed locally.
...
- Rollback Rule: On [FAILED], preserve all worktree changes and halt. Describe the affected files and request explicit user approval before any restore or discard action. If the failure blocks dependent tasks, mark those as [BLOCKED] instead of attempting them.
