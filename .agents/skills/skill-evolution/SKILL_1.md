...
4. Approve: Wait for explicit approval. Never edit the existing `SKILL_[n].md` in place, and never save the draft unapproved.
5. Apply: On approval, save the new `SKILL_[n].md` and update `.agents/INSTRUCTIONS.md` to point to it. Delegate all commit, push, and PR work to `semantic-git`.
