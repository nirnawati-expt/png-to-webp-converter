document.addEventListener("DOMContentLoaded", () => {
  ...
  function handleFiles(filesList) {
    ...
    if (combinedFiles.length > MAX_FILES) {
      showError(
        `Limit exceeded: You selected ${combinedFiles.length} files. Maximum is ${MAX_FILES}.`,
      );
    ...
    if (totalSize > MAX_SIZE_BYTES) {
      showError(
        `Limit exceeded: Total size is ${formatBytes(totalSize)}. Maximum is 50 MB.`,
      );
      fileInput.value = "";
      return;
    }
  ...
  function renderFileList() {
    ...
    downloadBtn.textContent =
      selectedFiles.length > 1 ? "> DOWNLOAD ALL (.zip)" : "> DOWNLOAD .WEBP";
  }
